// md2htmltemplates.go
// Demonstrates using progressive, self-contained functions:
...
// mdTest() does a simple Markdown to HTML conversion using app.mdToHTML
func mdTest() {
	var app = NewApp()
...
// mdYAMLTest() does a Markdown to HTML conversion using app.mdYAMLToHTML. Also reads front matter in YAML form.
func mdYAMLTest() {
	var app = NewApp()
...
// mdYAMLTemplateTest() does a Markdown to HTML conversion using app.mdYAMLToHTML, reads front matter in YAML form, and parses the template to show some parsed values interpolated into front matter.
...
// mdYAMLTemplateTest() does a Markdown to HTML conversion using app.mdYAMLToHTML, reads front matter in YAML form, parses the template to show some parsed values interpolated into front matter, and demonstrates a custom template function named ftime().
...
func main() {
  // 2 sections here: first section just loops through an array
  // of code fragments named test.
  // Second section demonstrates Markdown to HTML with increasing feature
  // support using discrete simple functions, including YAML front matter,
  // template suport, and Go template parsing with custom functions
	var app = NewApp()
	var err error
	var test = [...]string{`
    Month  using ftime: {{ ftime .Month }}
  `,
		`Default ftime with no params: {{ ftime }}
`,
	}
	var b []byte
	for _, v := range test {
		fmt.Println("")
		if b, err = app.mdYAMLToHTML([]byte(frontMatter + v + codeFence)); err != nil {
			fmt.Printf("\nmdYAMLToHTML failure\n%s\n", v)
			quit(err, 1)
		}
		// Works:
		var t string
		if t, err = app.doTemplateFuncs("templateTest", string(b)); err != nil {
			quit(err, 1)
		}
		fmt.Println(t)
